...
(function (window, document) {
  ...
  const QuickAdd = {
    ...
    rowCounter: 0,
    pending: 0,
    saves: new Map(),     // key -> { promise, resolve, payload, state }
    dupCache: new Map(),  // "supplierId|name" -> matches[]
    dupTimer: null,
    units: [],
    ...
    init(root) {
      ...
      if (finish) {
        finish.addEventListener("click", () => {
          ...
          this.allSettled().then(function () { window.location.href = "/purchases"; });
        });
      }
      ...
      // شبکه که برگشت، ردیف‌های ناموفق را خودکار دوباره می‌فرستیم
    ...
    bindKeyboard() {
      ...
      chain.forEach((el, index) => {
        ...
        el.addEventListener("keydown", (event) => {
          if (event.key === "Escape") { this.clearFields(true); return; }
          if (event.key !== "Enter") return;
          event.preventDefault();

          // تعداد + واحدِ ازپیش‌انتخاب‌شده = ثبت فوری، بدون رد شدن از بقیه فیلدها
          if (el === this.quantity && this.unit.value) { this.submit(); return; }
          if (el === this.description || index === chain.length - 1) { this.submit(); return; }

          const next = chain[index + 1];
          if (!next) { this.submit(); return; }
          next.focus();
    ...
    createSupplier() {
      ...
      if (!name) { this.newSupplierInput.focus(); return; }

      // خوش‌بینانه: همین حالا در لیست می‌آید و انتخاب می‌شود
      const option = document.createElement("option");
      option.value = "pending";
      option.textContent = name;
      this.supplier.insertBefore(option, this.supplier.lastElementChild);
      this.supplier.value = "pending";
      this.newSupplierBox.hidden = true;
      this.product.focus();

      request("/suppliers", { form: { name: name } })
        .then((result) => {
          if (!result.ok) {
            option.remove();
            this.supplier.value = this.lastSupplier || "";
            toast((result.data && result.data.message) || "تأمین‌کننده ثبت نشد", "error");
            return;
          }
          option.value = result.data.id;
          this.supplier.value = String(result.data.id);
          this.remember("pm.supplier", this.supplier.value);
          toast("تأمین‌کننده «" + result.data.name + "» اضافه شد", "success");
        })
        .catch(() => {
          option.remove();
          this.supplier.value = this.lastSupplier || "";
    ...
    bindDuplicates() {
      this.product.addEventListener("input", () => this.prefetchDuplicates());
    },

    dupKey(supplierId, name) {
      return String(supplierId) + "|" + name.trim().toLowerCase();
    },

    fetchDuplicates(supplierId, name) {
      const query = new URLSearchParams({ supplier: supplierId, product: name });
      return request("/check-duplicate?" + query.toString())
        .then(function (result) { return (result.data && result.data.matches) || []; })
        .catch(function () { return []; });
    },

    /** در حین تایپ، جواب «تکراری بودن» را از قبل می‌گیریم تا موقع ثبت صفر ثانیه معطلی باشد */
    prefetchDuplicates() {
      clearTimeout(this.dupTimer);
      const supplierId = this.supplier.value;
      const name = this.product.value.trim();
      if (!supplierId || supplierId === "__new__" || name.length < 2) return;

      const key = this.dupKey(supplierId, name);
      if (this.dupCache.has(key)) return;

      this.dupTimer = setTimeout(() => {
        this.fetchDuplicates(supplierId, name).then((matches) => this.dupCache.set(key, matches));
      }, 200);
    },
    ...
    submit() {
      ...
      const quantity = this.quantity.value.trim() || "1";   // خالی = ۱
      const description = this.description ? this.description.value.trim() : "";

      if (!supplierId || supplierId === "__new__") { this.shake(this.supplier); toast("اول تأمین‌کننده را انتخاب کن", "error"); return; }
      if (supplierId === "pending") { toast("تأمین‌کننده تازه هنوز ذخیره نشده — یک لحظه صبر کن", "warning"); return; }
      if (!name) { this.shake(this.product); this.product.focus(); return; }

      const known = this.dupCache.get(this.dupKey(supplierId, name));
      if (known && known.length > 0) {
        const ok = window.confirm(
          "«" + name + "» قبلاً ثبت شده:\n\n" + this.duplicateLines(known).join("\n") + "\n\nباز هم ثبت شود؟"
        );
        if (!ok) { this.product.select(); return; }
      }

      const item = {
        key: "tmp-" + (++this.rowCounter),
        supplier_id: supplierId,
        supplier_name: this.supplier.options[this.supplier.selectedIndex].textContent.trim(),
        product_name: name,
        quantity: quantity,
        unit: unit,
    ...
    save(key, payload, checkDuplicates) {
      ...
      const dupPromise = checkDuplicates
        ? this.fetchDuplicates(payload.supplier, payload.product)
        : Promise.resolve([]);

      request("/new-purchase", { form: payload })
        .then((result) => {
          this.pending -= 1;
          this.updateSync();

          if (!result.ok) {
            this.fail(key, (result.data && result.data.message) || "ثبت نشد");
            return;
          }
          ...
          if (liveRow) {
            ...
            this.setState(liveRow, "saved");
          }
          this.dupCache.delete(this.dupKey(payload.supplier, payload.product));
          ...
          dupPromise.then((matches) => {
            ...
            if (others.length) this.flagDuplicate(key, payload.product, others);
          });
        })
        .catch(() => {
          this.pending -= 1;
          this.updateSync();
          this.fail(key, "خطا در ارتباط با سرور");
        });
    },

    fail(key, message) {
      const row = this.rowByKey(key);
      if (row) this.setState(row, "failed");
      toast(message + " — ردیف قرمز را دوباره بفرست", "error");
    },

    retry(key) {
      const entry = this.saves.get(key);
      if (!entry || !entry.payload) return;
      this.save(key, entry.payload, false);
    },

    retryAllFailed() {
      const rows = this.body ? this.body.querySelectorAll("tr.row-failed") : [];
      rows.forEach((row) => this.retry(row.dataset.key));
    },
    ...
    allSettled() {
      ...
      this.saves.forEach(function (entry) {
        if (entry.promise) list.push(entry.promise);
      });
    ...
    rowByKey(key) {
      return this.body.querySelector('tr[data-key="' + key + '"]');
    },
    ...
    addRow(item) {
      ...
      row.innerHTML = this.rowHtml(item, "pending");
    ...
    rowHtml(item, state) {
      const key = item.key;
      return (
        '<td class="cell-status">' + this.statusHtml(key, state || "saved") + "</td>" +
        '<td class="cell-product"><span class="qa-name">' + escapeHtml(item.product_name) + "</span>" +
          '<span class="qa-sub">' + escapeHtml(item.supplier_name) + "</span></td>" +
        ...
        '<td class="cell-actions">' +
          '<button type="button" class="icon-btn" title="ویرایش" onclick="App.QuickAdd.startEdit(\'' + key + '\')">' +
          ...
          '<button type="button" class="icon-btn danger" title="حذف" onclick="App.QuickAdd.remove(\'' + key + '\')">' +
    ...
    statusHtml(key, state) {
      if (state === "pending") return '<span class="save-spinner" title="در حال ذخیره…"></span>';
      if (state === "failed") {
        return '<button type="button" class="icon-btn danger" title="ذخیره نشد — تلاش دوباره" onclick="App.QuickAdd.retry(\'' + key + '\')">' +
    ...
    setState(row, state) {
      ...
      if (cell) cell.innerHTML = this.statusHtml(row.dataset.key, state);
      if (state === "saved") {
    ...
    repaint(row, item) {
      row.innerHTML = this.rowHtml(item, row.dataset.state || "saved");
    },
    ...
    flagDuplicate(key, name, matches) {
      ...
      toast("«" + name + "» قبلاً هم ثبت شده بود", "warning");
    },
    ...
    startEdit(key) {
      ...
      row.querySelector(".cell-product").innerHTML =
        '<input class="field field-sm" style="width:150px" value="' + escapeAttr(item.product_name) + '">';
      row.querySelector(".cell-quantity").innerHTML =
        '<input class="field field-sm mono" style="width:78px" type="number" value="' + escapeAttr(item.quantity) + '">';
      ...
      row.querySelector(".cell-description").innerHTML =
        '<input class="field field-sm" style="width:160px" value="' + escapeAttr(item.description) + '">';
      row.querySelector(".cell-actions").innerHTML =
        '<button type="button" class="icon-btn" title="ذخیره" onclick="App.QuickAdd.saveEdit(\'' + key + '\')">' +
        ...
        '<button type="button" class="icon-btn" title="لغو" onclick="App.QuickAdd.cancelEdit(\'' + key + '\')">✕</button>';
    ...
    cancelEdit(key) {
      ...
      this.repaint(row, this.itemFromRow(row));
    },
    ...
    saveEdit(key) {
      ...
      const name = row.querySelector(".cell-product input").value.trim();
      ...
      if (!name || !quantity || !unit) { toast("اطلاعات محصول کامل نیست", "error"); return; }

      const prev = this.itemFromRow(row);
      const next = Object.assign({}, prev, {
        product_name: name, quantity: quantity, unit: unit, description: description
      });

      this.setRowData(row, next);
      row.classList.remove("editing");
      this.repaint(row, next);

      const entry = this.saves.get(key);
      if (entry && entry.payload) {
        entry.payload = Object.assign({}, entry.payload, {
          product: name, quantity: quantity, unit: unit, description: description
        });
      }

      this.whenSaved(key)
        .then(function (id) {
          return request("/product/" + id + "/edit", {
            form: { product: name, quantity: quantity, unit: unit, description: description }
          });
        })
        .then((result) => {
          if (result.ok) return;
          this.setRowData(row, prev);
          this.repaint(row, prev);
          toast((result.data && result.data.message) || "ویرایش ذخیره نشد", "error");
        })
        .catch(() => {
          this.setRowData(row, prev);
          this.repaint(row, prev);
          toast("ویرایش ذخیره نشد", "error");
        });
    },
    ...
    remove(key) {
      ...
      if (failed) { this.saves.delete(key); return; }

      this.whenSaved(key)
        .then(function (id) { return request("/product/" + id + "/delete", { method: "POST" }); })
        .then((result) => {
          if (result.ok) { this.saves.delete(key); return; }
          this.restore(parent, next, backup);
    ...
    updateCount() {
      ...
      if (this.count) this.count.textContent = rows;
    ...
    remember(storeKey, value) {
      try { window.localStorage.setItem(storeKey, value); } catch (err) { /* private mode */ }
      if (storeKey === "pm.supplier") this.lastSupplier = value;
    },
    ...
    focusStart() {
      if (this.supplier.value && this.supplier.value !== "__new__") this.product.focus();
      else this.supplier.focus();
    }
  };
  ...
  window.showError = function (text) { toast(text, "error"); };
  window.showSuccess = function (text) { toast(text, "success"); };
